...
`append` は「**後ろに足す**」。`sort()` は「**小さい順にならべる**」。

`(price, name)` は **2つで1組**。`sort()` は組の**左から**くらべるので、**値段を先に**。
...
`for price, name` と2つ書くと、組の左が `price`、右が `name` に入ります。
...
`for cat in totals:` は「辞書の**名札を1つずつ**」。`round(...)` は四捨五入。
