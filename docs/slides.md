...
**ノートブックを保存してください**（`Ctrl + S`）。

2コマ目は、ここに **値段** を足していきます。
...
いちばん上の `準備` から、もう一度 ▶ を押していってください。

ステップ2まで戻れば、`items` がまた使えます。すぐ終わります。

**エラーが出たら、1つ上のセルから実行しなおす。**
...
相手のサイトが変わっても、やることは同じ。違うのは **「どこを探すか」** だけ。
